...
class AddressAPITests(TestCase):
    ...
    def test_address_coordinates_must_be_inside_valid_ranges(self):
        ...
        self.assertIn("latitude", invalid_latitude.json())
        self.assertEqual(invalid_longitude.status_code, 400)
        self.assertIn("longitude", invalid_longitude.json())

    def test_address_coordinates_must_be_provided_together(self):
        response = self.client.post(
            "/api/v1/addresses/",
            {
    ...
    def test_address_accepts_and_returns_boundary_coordinates(self):
        ...
        self.assertEqual(response.data[0]["latitude"], "-90.0000000")
        self.assertEqual(response.data[0]["longitude"], "180.0000000")
...
class LocationManagementAPITests(TestCase):
    ...
    def test_deleted_courier_profile_does_not_block_service_city_deletion(self):
        ...
        courier = User.objects.create_user(
            username="deleted_city_courier",
            email="deleted-city-courier@example.com",
            phone="+201000000189",
            password="Passw0rd!",
            role=User.Role.REPRESENTATIVE,
        )
    ...
    def test_deleted_client_relations_do_not_block_service_city_deletion(self):
        city = ServiceCity.objects.create(name="Deleted Client City")
        client = User.objects.create_user(
            username="deleted_city_client",
            email="deleted-city-client@example.com",
            phone="+201000000190",
            password="Passw0rd!",
            role=User.Role.CLIENT,
        ...
        self.assertIsNone(client.market_region_mode)
